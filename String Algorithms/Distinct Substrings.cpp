...
void radix_sort(vector<suffix> &suffixes, int inx) {
    ...
    int n = suffixes.size();
    //vector<int> count(258, 0);
    //bcz rank may have upto n
...
vector<int> build_lcp(string &text, vector<int> &suffix_array) {
    int n = text.size();
...
void solve_problem() {
    ...
    vector<int> lcp = build_lcp(text, suffix_array);
