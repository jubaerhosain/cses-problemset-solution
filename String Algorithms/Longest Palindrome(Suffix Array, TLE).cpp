...
void solve_problem() {
    ...
    string total = text + "$" + rev;

    vector<int> suffix_array = build_suffix_array(total);
    vector<int> lcp_array = build_lcp_array(total, suffix_array);
    ...
    cout << text.size() << endl;
    cout << total.substr(pos, mx_len) << endl;
}
